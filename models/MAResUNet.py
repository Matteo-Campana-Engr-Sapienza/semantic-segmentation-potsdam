...
class MAResUNet18(nn.Module):
    def __init__(self, num_channels=3, num_classes=6,
        nonlinearity =  partial(F.relu, inplace=True),
        pretrain = "imagenet"):

        super(MAResUNet18, self).__init__()
        self.name = 'MAResUNet18'
        ...
        if pretrain == "imagenet":
            ...
            base_model = tmodels.resnet18(pretrained = False)
...
def MAResUNet18_(img_channel=3,num_classes = 6):
    return MAResUNet18(img_channel, num_classes = num_classes)
# <---------------------------------------------------------------------------->
# MAResUNet VARIATIONAL AUTO ENCODER FULL MODEL PRE-TRAINING
# <---------------------------------------------------------------------------->
...
def MAResUNet50VAE(img_channel=3):
    return MAResUNet(block, decoder_block, [3, 6, 4, 3], img_channel)

def MAResUNet101VAE(img_channel=3):
    return MAResUNet(block, decoder_block, [3, 23, 4, 3], img_channel)

def MAResUNet152VAE(img_channel=3):
    return MAResUNet(block, decoder_block, [3, 36, 8, 3], img_channel)


# <---------------------------------------------------------------------------->
# MAResUNet VARIATIONAL AUTO ENCODER FULL MODEL SEMANTIC SEGMENTATION
# <---------------------------------------------------------------------------->

class MAResUNet_SS(nn.Module):
    def __init__(self, block ,decoder_block, layers, image_channels):
        super(MAResUNet_SS, self).__init__()
...
def MAResUNet50VAE_SS(img_channel=3):
    return MAResUNet_SS(block, decoder_block, [3, 6, 4, 3], img_channel)

def MAResUNet101VAE_SS(img_channel=3):
    return MAResUNet_SS(block, decoder_block, [3, 23, 4, 3], img_channel)

def MAResUNet152VAE_SS(img_channel=3):
    return MAResUNet_SS(block, decoder_block, [3, 36, 8, 3], img_channel)
